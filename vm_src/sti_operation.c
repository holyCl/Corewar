...
static int			sti_validation(int *args, unsigned int *tmp_pos)
{
	unsigned int	step;
	unsigned int	i;
...
static void			get_all_arguments_sti(t_vm *vm, int *args_array,
...
static void			store_value(t_vm *vm, unsigned int array, int position,
					unsigned char color)
{
	unsigned int	i;
...
static int			calculate_position(t_pc *process, int *args_array,
					unsigned int *args)
{
	int				pos;
	...
	if (args_array[1] == REG_CODE && args[1] >= 1 && args[1] <= 16)
	{
		if (args_array[2] == REG_CODE && args[2] >= 1 && args[2] <= 16)
			pos = ((int)REG[args[1] - 1] + (int)REG[args[2] - 1]) % IDX_MOD;
		else
			pos = ((int)REG[args[1] - 1] + (short)args[2]) % IDX_MOD;
	}
	else if (args_array[1] == IND_CODE)
	{
		if (args_array[2] == REG_CODE && args[2] >= 1 && args[2] <= 16)
			pos = ((int)args[1] + (int)REG[args[2] - 1]) % IDX_MOD;
		else
			pos = ((int)args[1] + (int)args[2]) % IDX_MOD;
	}
	else
	{
		if (args_array[2] == REG_CODE && args[2] >= 1 && args[2] <= 16)
			pos = ((short)args[1] + (int)REG[args[2] - 1]) % IDX_MOD;
		else
			pos = ((short)args[1] + (short)args[2]) % IDX_MOD;
	}
	return (pos);
}
...
void				sti_op(t_vm *vm, t_pc *process)
{
	...
	if (sti_validation(args_array, &tmp_pos))
	{
		...
		position = (POS + calculate_position(process, args_array, args));
		if (position < 0)
			position = MEM_SIZE + (position % MEM_SIZE);
		else
			position %= MEM_SIZE;
		if (args[0] >= 1 && args[0] <= 16)
			store_value(vm, REG[args[0] - 1], position, process->color);
	}
